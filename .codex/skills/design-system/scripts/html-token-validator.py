...
Source of truth: assets/design-tokens.css (or a path passed with --tokens-css)
...
def discover_project_root(start: Path) -> Path:
    """Find the nearest project root that contains assets/design-tokens.css."""
    current = start.resolve()
    for candidate in [current, *current.parents]:
        if (candidate / 'assets' / 'design-tokens.css').exists():
            return candidate
...
def get_asset_dirs(project_root: Path):
    return {
...
def load_css_variables(tokens_css_path: Path) -> Dict[str, str]:
...
Examples:
  ...
  %(prog)s --tokens-css path/to/design-tokens.css
"""
    )
    ...
    parser.add_argument('--project-root', help='Project root containing assets/design-tokens.css')
    parser.add_argument('--tokens-css', help='Explicit path to design-tokens.css')

    args = parser.parse_args()
    project_root = Path(args.project_root).resolve() if args.project_root else discover_project_root(Path.cwd())
    tokens_css_path = Path(args.tokens_css).resolve() if args.tokens_css else project_root / 'assets' / 'design-tokens.css'
    asset_dirs = get_asset_dirs(project_root)

    # Show colors mode
    if args.colors:
        variables = load_css_variables(tokens_css_path)
